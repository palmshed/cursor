...
### 6.2 Priority 2: Directory-Aware Find / Scan (Complete)
* **Status:** **Complete.** See `docs/telemetry/directory_aware_find_report.md`.
...
| Query | Pre-Fix Tool Sequence | Pre-Fix Outcome | Post-Fix Tool Sequence | Post-Fix Outcome |
|-------|----------------------|----------------|----------------------|----------------|
| `where is replay implemented` | `find replay` → `read` | Success (already worked) | `find replay` → `read` | Success (unchanged) |
| `find cursor binary` | `find cursor[_-]?binary` → *no match* → `grep cursor binary` → *no match* → `read` → *no files* | InsufficientEvidence | `find cursor[_-]?binary` → **word-level match** on `cursor_binary` stem → `read` | Success |
| `find cursor bin` | `find cursor[_-]?bin` → *no match* → `grep cursor bin` → *no match* → `read` → *no files* | InsufficientEvidence | `find cursor[_-]?bin` → **word-level match** on `cursor_binary` stem → `read` | Success |
| `where is CommandRouter implemented` | `find CommandRouter` → exact match → `read` | Success (already worked) | `find CommandRouter` → exact match → `read` | Success (unchanged) |

**New estimated clean developer distribution:**

| Outcome | Post-Fix (Routing) | Post-Fix (Routing + Find) | Delta |
| :--- | :---: | :---: | :---: |
| **Success** | 199 (91.7%) | **203 (93.5%)** | +4 resolved |
| **Insufficient Evidence** | 14 (6.5%) | **10 (4.6%)** | -4 resolved |
| **Failure** | 4 (1.8%) | **4 (1.8%)** | unchanged |

**Metrics capture:**

```
filename_hits:     replay → 4 candidates (unchanged)
                   CommandRouter → exact match (unchanged)
                   cursor[_-]?binary → 0 → 1 candidate (fixed)
symbol_hits:       no change
grep_hits:         replay → 0 (find succeeded, unchanged)
                   cursor[_-]?binary → 0 grep eliminated (find now succeeds)
directory_hits:    no change
```

**Retrieval efficiency:**
- `find cursor binary` before: 3 tools (find+grep+read), ~200ms, 0 files read
- `find cursor binary` after: 2 tools (find+read), ~150ms, 1+ files read
- 33% tool reduction, 100% file-read improvement

### 7.6 Updated Bottleneck (After Directory-Aware Find)

Remaining **4.6% `insufficient_evidence`** (10 traces):
* `where is replay implemented` (8x) — *already resolved by routing/meta fixes, not find*
* `where is CommandRouter implemented` (2x) — *already worked*
* *Other synthetic/nonexistent-symbol queries excluded from clean developer count*
